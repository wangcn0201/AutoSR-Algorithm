...
class AutoSR_Graph(object):
	def __init__(self, config_path, data_name, gpu_device, evaluation_function):
		...
		for i in range(len(key_name_list)):
			...
			if pre_key_types != None:
				for j in range(len(pre_key_types)):
					pre_key_type = pre_key_types[j]
					if pre_key_type not in LinearRelations.keys():
						LinearRelations[pre_key_type] = []
					for k in range(len(key_types)):
						key_type = key_types[k]
						LinearRelations[pre_key_type].append(key_type)
			pre_key_types = list(key_types)
		logging.info('LinearRelations = %s', str(LinearRelations))
		...
		self.key_name_list = BuildReverseGraph(Parameters, Options, LinearRelations)
		self.Parameters, self.Options, self.Relations = Parameters, Options, LinearRelations
		...
		self.model = GraphModel(node_num, self.Parameters, self.Options, self.Relations).cuda()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/NAS_AutoSR_Graph-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
	...
	def main(self):
		...
		# initialize memory
		logging.info('\ninitialize memory......')
		random_code_list = RandomCode(self.batch_size, self.Parameters, self.Options, self.Relations)
		logging.info('random_code_list: %s', str(random_code_list))
