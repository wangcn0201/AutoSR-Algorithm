...
from autosr.GraphModel_EdgeWeight import *
from autosr.ReplayMemory import *


class AutoSR_EdgeWeight(object):
	def __init__(self, config_path, data_name, gpu_device, evaluation_function):
		super(AutoSR_EdgeWeight, self).__init__()
		...
		self.model = GraphModel_EdgeWeight(edge_num, Parameters, Options, Relations).cuda()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/NAS_AutoSR_EdgeWeight-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
