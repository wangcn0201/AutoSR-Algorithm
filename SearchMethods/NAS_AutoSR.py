...
class AutoSR(object):
	def __init__(self, config_path, data_name, gpu_device, evaluation_function):
		...
		self.model = GraphModel(node_num, Parameters, Options, Relations).cuda()
	...
	def environment_setting(self, config):
		...
		logging_path = logging_dir + '/NAS_AutoSR-Run-{}'.format(time.strftime("%Y%m%d-%H%M%S"))
	...
	def main(self):
		...
		# initialize memory
		logging.info('\ninitialize memory......')
		random_code_list = RandomCode(self.batch_size//2, Parameters, Options, Relations)
