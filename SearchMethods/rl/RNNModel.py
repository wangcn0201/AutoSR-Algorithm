...
class RNNModel(torch.nn.Module):
	def __init__(self, search_space, action_list, controller_hid=100, cuda=True, mode="train",
	...
	def _construct_action(self, actions):
		...
		for single_action in actions:
			...
			for action, action_name in zip(single_action, self.action_list):
				...
				model_code[action_name] = action_content
	...
	def sample(self, batch_size=1, with_details=False):
		...
		model_code_list = self._construct_action(actions)

		if with_details:
			return model_code_list, torch.cat(log_probs), torch.cat(entropies)
		return model_code_list
